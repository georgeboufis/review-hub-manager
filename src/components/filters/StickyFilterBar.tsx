...
export default function StickyFilterBar() {
  ...
  const sources: { key: string; label: string; colorVar: string }[] = [
    { key: 'google', label: 'Google', colorVar: '--brand-google' },
    { key: 'booking', label: 'Booking', colorVar: '--brand-booking' },
              ...
              {sources.map((s) => {
                ...
                return (
                  <Button
                    ...
                    style={active ? { background: `hsl(var(${s.colorVar}))` } : {}}
              ...
              {[1, 2, 3, 4, 5].map((r) => {
                ...
                return (
                  <Button
                    ...
                    variant={active ? 'default' : 'outline'}
                    className="rounded-full px-3 h-8"
                    onClick={() => setActiveRatings((prev) => toggle(prev, r))}
                  >
                    {r}★
                  </Button>
                );
